...
test.describe('voucher editor', () => {
  test('meno editor has Kitsas tabs, type icon, and Valmis off until complete', async ({
    ...
    await page.locator('.assistant-vat .icon-select-btn').click()
    await expect(page.locator('.icon-select-list [data-vat-icon="sales-netto"]').first()).toBeVisible()
  ...
  test('Lisää rivi appends a selected table row and keeps the previous line', async ({
    ...
    await expect(table.locator('tbody tr').nth(1)).toContainText('25,5 %')
  ...
  test('euro fields keep one decimal and allow typing in Veroton', async ({ page }) => {
    ...
    const alv = page.locator('.assistant-vat .icon-select-btn')
